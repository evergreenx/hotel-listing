...
export default function HotelModal({ isOpen, onClose, categories, countries, hotel }: HotelModalProps) {
  const dispatch = useDispatch()
  const { control, handleSubmit, reset, formState: { errors } } = useForm<Hotel>({
    defaultValues: hotel || {
      id: '',
      name: '',
      country: '',
                ...
                <Select onValueChange={field.onChange} value={field.value}>
                  ...
                  <SelectContent>
                    {countries.map((country, index) => (
                      <SelectItem key={index} value={country.country}>{country.country}</SelectItem>
                    ))}
                  </SelectContent>
                </Select>
              )}
